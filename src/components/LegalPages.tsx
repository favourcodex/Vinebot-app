...
const LegalLayout: React.FC<LegalLayoutProps> = ({ title, icon, onBack, children }) => {
  return (
    <div className="bg-[#050505] min-h-screen text-neutral-200 py-12 px-4 sm:px-6 lg:px-8 font-sans relative">
      <div className="max-w-4xl mx-auto relative">
        <button
          onClick={onBack}
          className="inline-flex items-center gap-2 text-xs text-neutral-400 hover:text-white mb-8 transition cursor-pointer"
        >
          <ArrowLeft className="w-4 h-4" /> Return to VIN-CORP
        </button>

        <div className="flex items-center gap-3 border-b border-[#262626] pb-6 mb-8">
          <div className="p-2.5 bg-[#121212] border border-[#262626] rounded-xl text-white">
            {icon}
          </div>
          <div>
            <h1 className="text-2xl font-extrabold text-white tracking-tight uppercase">{title}</h1>
            <p className="text-[10px] text-neutral-500 tracking-wider uppercase font-mono mt-0.5">Official Platform Policy</p>
          </div>
        </div>

        <div className="bg-[#121212] border border-[#262626] rounded-2xl p-6 sm:p-10 shadow-2xl space-y-6 text-sm leading-relaxed text-neutral-300">
          {children}
        </div>

        <div className="text-center mt-12 text-[10px] text-neutral-500 uppercase font-mono tracking-wider">
          VIN-CORP Compliance Console &bull; Security Operations Guard
        </div>
      </div>
    </div>
  );
...
export const TermsPage: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  return (
    <LegalLayout title="Terms & Conditions" icon={<Scale className="w-6 h-6 text-white" />} onBack={onBack}>
      <div className="space-y-6">
        <div className="border-b border-[#262626] pb-4 mb-4">
          <h2 className="text-lg font-bold text-white uppercase font-mono">TERMS AND CONDITIONS OF USE: VIN-CORP</h2>
        ...
        <div className="space-y-4">
          ...
          <p className="text-neutral-300">
            VIN-CORP is a software development and algorithmic technology provider. VIN-CORP is not a registered broker-dealer, financial advisor, investment analyst, or tax planner. All software, tools, signals, parameters, and documentation provided are strictly for informational and educational purposes. VIN-CORP does not solicit, recommend, or provide investment advice. Any automated trading settings, indicators, or strategies you configure are executed solely at your own discretion and risk.
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <p className="text-neutral-300">
            By utilizing VIN-CORP's cloud hosting, VPS, or MetaTrader 5 (MT5) execution bridging technology, you assume all operational and technical risks. This includes, but is not limited to, API connection latency, internet connection failures, VPS downtime, third-party software bugs, server crashes, execution slippage, broker-side rejection of orders, and algorithmic errors. VIN-CORP is not responsible for trade execution failures resulting from technical anomalies.
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <p className="text-neutral-300">
            To the maximum extent permitted by applicable law, in no event shall VIN-CORP, its affiliates, directors, employees, or technology providers be liable for any direct, indirect, incidental, special, exemplary, punitive, or consequential damages (including, but not limited to, loss of trading capital, trading losses, loss of profits, data corruption, or system outages) arising out of or in connection with the use, performance, or inability to use the VIN-CORP platform, regardless of the legal theory.
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <p className="text-neutral-300">
            The VIN-CORP platform, including all algorithms, VPS hosting configurations, templates, and execution scripts, is provided on an "AS IS" and "AS AVAILABLE" basis without warranties of any kind, either express or implied. VIN-CORP disclaims all warranties, including but not limited to, implied warranties of merchantability, fitness for a particular purpose, non-infringement, or uninterrupted, bug-free operation.
          </p>
        </div>
...
export const PrivacyPage: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  return (
    <LegalLayout title="Privacy Policy" icon={<FileText className="w-6 h-6 text-white" />} onBack={onBack}>
      <div className="space-y-6">
        <div className="border-b border-[#262626] pb-4 mb-4">
        ...
        <div className="space-y-4">
          ...
          <p className="text-neutral-300">
            VIN-CORP does not sell or distribute personal information. We proxy payment details through Stripe's certified PCI-compliant gateway. No payment card numbers are ever processed or stored on our servers.
          </p>
        </div>
      </div>
    </LegalLayout>
...
export const CookiePolicyPage: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  return (
    <LegalLayout title="Cookie Policy" icon={<HelpCircle className="w-6 h-6 text-white" />} onBack={onBack}>
      <div className="space-y-6">
        <div className="border-b border-[#262626] pb-4 mb-4">
        ...
        <p className="text-neutral-300">
          This policy details how VIN-CORP utilizes cookies to maintain secure authenticated user sessions and verify portal integrity.
        </p>
        ...
        <div className="border-[#262626] border bg-[#181818] p-5 rounded-xl flex items-start gap-3 text-sm">
          ...
          <div>
            ...
            <p className="text-xs mt-1 text-neutral-300">
              By connecting your MT5 broker account to VIN-CORP's automated algorithms, you expose your capital to active market conditions that can result in sudden, complete depletion of capital.
            </p>
          </div>
        </div>
