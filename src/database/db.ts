...
class VinebotDatabase {
  ...
  public getSubscriptionPlans(): SubscriptionPlan[] {
    ...
    if (!this.state.subscriptionPlans || this.state.subscriptionPlans.length < 2 || !this.state.subscriptionPlans.some(p => p.id === 'plan-vip-month')) {
      ...
      // Ensure cached plans have current active Stripe Price IDs
      let modified = false;
      this.state.subscriptionPlans = this.state.subscriptionPlans.map(p => {
        if (p.id === 'plan-premium-month' || p.id === 'plan-pro-month') {
          if (p.stripePriceId !== proPriceId) {
            modified = true;
            return { ...p, name: 'Vinebot Pro Access', stripePriceId: proPriceId };
          }
        }
        if (p.id === 'plan-vip-month') {
          if (p.stripePriceId !== vipPriceId) {
            modified = true;
            return { ...p, name: 'Vinebot VIP Unlimited', stripePriceId: vipPriceId };
          }
        }
        return p;
      });
